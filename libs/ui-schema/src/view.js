...
const viewProps = task(
  t => (props) => {
    ...
    const node = t.find(
      prop => t.equals(t.head(prop), '@node'),
      propsPairs,
    )
    // ui -> filter ui components:
    const uiList = t.filter(
      prop => !t.equals(t.head(prop), '@node')
        && t.equals(t.head(t.head(prop)), '@'),
      propsPairs,
    )
    ...
    const omitList = t.map(
      prop => t.head(prop),
      t.concat(uiList, handlerList),
    )
    const nextProps = t.omit(
      t.concat([ '@node' ], omitList),
      props,
    )
    // omit tokens in keys:
    const omitToken = prop => [
      `${t.tail(t.head(prop))}`, t.head(t.tail(prop)),
    ]
    // yield:
    return {
...
const viewNode = task(
  t => (name = 'view', props = {}, children = []) => {
    const nextProps = viewProps(props)
    const nextChildren = t.equals(t.type(children), 'Array')
      ? children
      : []
    const childrenMap = t.mapObjIndexed(
      (value, key) => t.path([ key ], t.head(value)),
      t.groupBy(
        child => t.head(t.head(t.toPairs(child))),
      )(nextChildren),
    )
    return {
      [name]: t.merge(
...
// exports
export const viewSchema = factory => factory(
  viewNode,
  task(
    t => t.mergeDeepRight(
      VIEW_SCHEMA,
