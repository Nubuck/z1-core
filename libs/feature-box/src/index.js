...
export const createStateBox = core.task(t => props => {
  if (t.isType(t.path(['routes'], props), 'Function')) {
  ...
  return createStateBox(
    t.mergeAll([
      ...
      t.omit(['initial'], props || {}),
      {
        ...
        routes(r, actions) {
          return t.flatten(
            t.map(route => route(r, actions))(nextParts.routes || [])
          )
        },
        guards(g, box) {
          return t.flatten(
...
export const combineStateBoxes = core.task(t => boxes =>
  core.combineStateBoxes(boxes, (nextBoxes, box) => {
    return {
...
export const createStateStore = core.task(t => (props = {}) => {
...
export const reloadStateStore = (store, boxes) =>
...
export const renderRoute = task(t => (actionType, routes) => {
  const matchedDef = t.find(
    routeDef =>
      t.gt(
        t.findIndex(
          type => t.eq(actionType, type),
          t.isType(routeDef.type, 'Array') ? routeDef.type : [routeDef.type]
        ),
        -1
      ),
    routes
...
export const connectState = task(t => (selector, mutations = undefined) =>
  connect(
    selector,
    t.notType(mutations, 'Object')
      ...
      : dispatch => {
          return {
            dispatch,
            mutations: bindActionCreators(mutations, dispatch),
          }
        }
  )
)
