...
const hookSignature = task(t => def =>
  t.has('before')(def) || t.has('after')(def) || t.has('error')(def)
)

const hookAndEventSignature = task(t => def =>
  t.has('hooks')(def) || t.has('events')(def)
)

const safeServiceName = task(t => path =>
...
const pathIncludeNamespace = task(t => (path, namespace) => {
  if (t.not(path)) {
    return false
  }
