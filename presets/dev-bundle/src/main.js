...
// with babel
const babelCommand = task(
  t => (output = 'build') => {
    const libDir = Fs.cwd(__dirname, '..')
    const base = [
      `--presets ${libDir.path('node_modules', 'babel-preset-react')}`,
      `--plugins ${libDir.path('node_modules', 'babel-plugin-transform-object-rest-spread')}`,
      '--no-babelrc',
      `-d ${output}`,
      `--ignore '*.test.js'`,
      `${Fs.path('src')}`,
...
async function compileList(build = false, watch = false, external = false) {
  const entry = build
    ? 'build'
    : 'src'
  ...
  await task(
    (t,a) => a.map(list, 1, async file => {
      try {
        const result = await Execa.shell(bundleCommand(entry, file, watch, external), { cwd: Fs.cwd() })
        process.stdout.write(`\nComplete bundling -> ${result.stdout}`)
      }
      catch (e) {
        process.stdout.write(`\nError in bundling -> ${e}`)
