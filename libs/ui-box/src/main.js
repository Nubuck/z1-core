...
const withScreenSize = task(
  t => (factory) => v => {
    const nextVal = t.equals(t.type(v), 'Array')
    && t.equals(t.length(v), 1)
      ? t.head(v)
      : v
...
const classNames = {
  display: withScreenSize(
    (value, size) => size && size !== 'xs'
      ? `d-${size}-${value}`
      : `d-${value}`,
  ),
  flex: withScreenSize(
    (value, size) => {
      const nextValue = value === 'col'
        ? 'column'
        : value
      return size && size !== 'xs' ? `flex-${size}-${nextValue}`
        : `flex-${nextValue}`
    },
  ),
  flexItems: withScreenSize(
    (value, size) => size && size !== 'xs'
      ? `align-items-${size}-${value}`
      : `align-items-${value}`,
  ),
  flexContent: withScreenSize(
    (value, size) => size && size !== 'xs'
  ...
  pin: task(
    t => v => {
      if (t.type(v) === 'Array') {
        const nextVal = t.length(v) === 1
          ? t.head(v)
          : v
        const nextList = t.map(
          p => `${p}-0`,
          nextVal,
        )
    ...
    const classNameList = t.filter(
      ...
      t.map(
        prop => {
          ...
          return t.has(head)(classNames)
            ? classNames[ head ](tail)
            : ''
        },
        boxList,
      ),
...
export const uiBox = task(
  t => (box) => {
    box = t.has('display')(box)
      ? box
      : nextBox(box, { display: 'flex' })
    return {
      next: (b) => uiBox(nextBox(box, b)),
      set: (prop, value) => uiBox(
        nextBox(box, { [prop]: value }),
      ),
