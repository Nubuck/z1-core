...
export const field = fn(
  (t) =>
    (nameOrProps, propsOrChildren, childrenOrAdditional, otherAdditional) => {
      ...
      // next
      const fieldKey =
        t.atOr(false, 'keepKeyCase', props) === true
          ? name
          : t.to.camelCase(name)
      ...
      const fieldType = t.eq(baseType, 'array')
        ? isFixedItemList(children, additional)
          ...
          : baseType
        : baseType
      const fieldProps = t.omit(['ui', 'required', 'keepKeyCase'], props)
      // yield
      return (ctx, parent) => {
        const xProps = t.atOr(
